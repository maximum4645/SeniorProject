...
This script controls two SG90-like servos on channels 4 and 8 of a PCA9685 board.
It uses the adafruit_motor.servo module to automatically convert angles into the
correct PWM signal. The servo on channel 4 moves between 0° (closed) and 90° (open),
...
# 2) Create servo objects on channels 4 and 8
...
#    Channel 4: moves from 0° (closed) to 90° (open)
SERVO_4_CLOSED = 0
SERVO_4_OPEN   = 90
...
try:
    while True:
        ...
        servo4.angle = SERVO_4_CLOSED
        servo8.angle = SERVO_8_CLOSED
        print("Flaps closed: Servo 4 -> 0°, Servo 8 -> 180°")
        time.sleep(1)  # Give time for movement

        # Move servos to the "open" position
        servo4.angle = SERVO_4_OPEN
        servo8.angle = SERVO_8_OPEN
        print("Flaps open: Servo 4 -> 90°, Servo 8 -> 90°")
        time.sleep(1)
